...
Titulo: Atualiza todas as tabelas dos banco de dados.
...
WHERE TB.[idBaseDeDados] = D.[idBaseDeDados]
  AND TB.[schema_name] COLLATE Latin1_General_CI_AS = R.Schema_Name	
  AND TB.[table_name]  COLLATE Latin1_General_CI_AS = R.Table_Name
  AND(TB.[reservedkb] <> CAST(R.[Reservado(KB)] AS REAL)
   OR TB.[datakb]     <> CAST(R.[Dados(KB)] AS REAL)
   OR TB.[Indiceskb]  <> CAST(R.[Indices(KB)] AS REAL)
   OR TB.[sumline]    <> R.Qtd_Linhas)
				'
