...
'
						...
						--BEGIN
							SET @ExeScript = '
INSERT INTO [SGBD].[MtPgTableIndexStat]
           ([idSGBDTableIndex]
           ,[idx_scan]
           ,[idx_tup_read]
           ,[idx_tup_fetch])
				SELECT ID.[idSGBDTableIndex]
						, A.idx_scan
						, A.idx_tup_read
						, A.idx_tup_fetch
				FROM OPENQUERY(LinkedServerJob'+@LinkdName+', '' SELECT schemaname, 
																		relname, 
																		indexrelname, 
																		idx_scan, 
																		idx_tup_read, 
																		idx_tup_fetch
																 FROM pg_catalog.pg_stat_all_indexes '') AS A
				...
				INNER JOIN [SGBD].[SGBDTable] AS TB ON TB.[idDatabases] = D.idDatabases  
												   ...
												   AND TB.[table_name]   COLLATE Latin1_General_CI_AS = A.relname
				INNER JOIN [SGBD].[SGBDTableIndex] AS ID ON ID.[idSGBDTable] = TB.[idSGBDTable]
				                                        AND ID.[Index_name] COLLATE Latin1_General_CI_AS  = A.indexrelname '
