...
/****** Object:  StoredProcedure [dbo].[SP_SlowQueryTime]    Script Date: 19/03/2020 16:55:59 ******/
SET ANSI_NULLS ON
GO
...
-- =============================================
/**/
CREATE PROCEDURE [dbo].[SP_SlowQueryTime]
AS
BEGIN
/* *************************************************************************************************/
...
select  
	 ...
	 , CONVERT(TIME, DATEADD(SECOND, round((cast(total_elapsed_time / (execution_count + 0.0) as money) / 1000000),2), 0), 114) as 'MedioTempoExecQuery' 
	 , CONVERT(TIME, DATEADD(SECOND, round((cast(total_elapsed_time as money) / 1000000),2), 0), 114)                           as 'SomaTempoExecQuery'	 
